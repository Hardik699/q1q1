...
export default function MasterAdmin() {
  ...
  const loadAllData = () => {
    try {
      ...
      const systemAssets = JSON.parse(localStorage.getItem("systemAssets") || "[]");
      const pcLaptopAssets = JSON.parse(localStorage.getItem("pcLaptopAssets") || "[]");
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        <header className="flex items-center justify-between">
          <div>
            ...
            <p className="text-slate-400">Complete database overview in table format</p>
          </div>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-white">{masterData.employees.length}</p>
                </div>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-white">{masterData.systemAssets.length}</p>
                </div>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-white">{masterData.pcLaptopAssets.length}</p>
                </div>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-white">{masterData.itAccounts.length}</p>
                </div>
        ...
        <Card className="bg-slate-900/50 border-slate-700 backdrop-blur-sm">
          <CardHeader>
            <CardTitle className="text-white">Complete Database Tables</CardTitle>
          </CardHeader>
          <CardContent>
            <Tabs defaultValue="employees" className="w-full">
              <TabsList className="grid w-full grid-cols-5 lg:grid-cols-9 bg-slate-800 mb-6">
                ...
                <TabsTrigger value="admin" className="text-xs">Admin Users</TabsTrigger>
                <TabsTrigger value="departments" className="text-xs">Departments</TabsTrigger>
                <TabsTrigger value="systemassets" className="text-xs">System Assets</TabsTrigger>
                <TabsTrigger value="pclaptops" className="text-xs">PC/Laptops</TabsTrigger>
                <TabsTrigger value="itaccounts" className="text-xs">IT Accounts</TabsTrigger>
                <TabsTrigger value="salaries" className="text-xs">Salaries</TabsTrigger>
                <TabsTrigger value="leaves" className="text-xs">Leave Requests</TabsTrigger>
                <TabsTrigger value="notifications" className="text-xs">IT Notifications</TabsTrigger>
              </TabsList>
              ...
              <TabsContent value="admin">
                <div className="rounded-md border border-slate-700">
                    ...
                    <TableBody>
                      {masterData.adminUsers.map((user) => (
                        <TableRow key={user.id}>
                          ...
                          <TableCell>
                            <Badge variant={masterData.userCredentials[user.username] ? "default" : "destructive"}>
                  ...
                  {masterData.adminUsers.length === 0 && (
                    <div className="p-8 text-center text-slate-400">No admin users found</div>
                  )}
                </div>
              </TabsContent>
                    ...
                    <TableBody>
                      {masterData.pcLaptopAssets.map((pc) => (
                        <TableRow key={pc.id}>
                          ...
                          <TableCell className="text-xs">{pc.mouseId ? getAssetDetails(pc.mouseId) : "-"}</TableCell>
                          <TableCell className="text-xs">{pc.keyboardId ? getAssetDetails(pc.keyboardId) : "-"}</TableCell>
                          <TableCell className="text-xs">{pc.motherboardId ? getAssetDetails(pc.motherboardId) : "-"}</TableCell>
                          <TableCell className="text-xs">{pc.ramId ? getAssetDetails(pc.ramId) : "-"}</TableCell>
                          <TableCell className="text-xs">{pc.ramId2 ? getAssetDetails(pc.ramId2) : "-"}</TableCell>
                          <TableCell className="text-xs">{pc.storageId ? getAssetDetails(pc.storageId) : "-"}</TableCell>
                          <TableCell>{new Date(pc.createdAt).toLocaleDateString()}</TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
                  </Table>
                  {masterData.pcLaptopAssets.length === 0 && (
                    <div className="p-8 text-center text-slate-400">No PC/Laptop configurations found</div>
                  )}
                </div>
              </TabsContent>
              ...
              <TabsContent value="itaccounts">
                <div className="rounded-md border border-slate-700">
                    ...
                    <TableBody>
                      {masterData.itAccounts.map((account) => (
                        <TableRow key={account.id}>
                          ...
                          <TableCell className="font-mono">{account.systemId}</TableCell>
                          <TableCell>{account.tableNumber}</TableCell>
                          <TableCell>{account.department}</TableCell>
                          <TableCell>
                            <div className="text-xs">
                              {account.emails.map((email, idx) => (
                                <div key={idx}>{email.provider}: {email.email}</div>
                              ))}
                            </div>
                          </TableCell>
                          <TableCell className="text-xs">{account.vitelGlobal.provider}: {account.vitelGlobal.id}</TableCell>
                          <TableCell className="text-xs">ID: {account.lmPlayer.id}<br/>License: {account.lmPlayer.license}</TableCell>
                  ...
                  {masterData.itAccounts.length === 0 && (
                    <div className="p-8 text-center text-slate-400">No IT accounts found</div>
                  )}
                </div>
              </TabsContent>
                    ...
                    <TableBody>
                      {masterData.salaryRecords.map((salary) => (
                        <TableRow key={salary.id}>
                          ...
                          <TableCell>{salary.actualWorkingDays}/{salary.totalWorkingDays}</TableCell>
                          <TableCell>₹{salary.basicSalary.toLocaleString()}</TableCell>
                          <TableCell>₹{salary.bonus?.toLocaleString() || 0}</TableCell>
                          <TableCell>₹{salary.deductions?.toLocaleString() || 0}</TableCell>
                          <TableCell className="font-medium">₹{salary.totalSalary.toLocaleString()}</TableCell>
                          <TableCell>{salary.paymentDate ? new Date(salary.paymentDate).toLocaleDateString() : "-"}</TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
                  </Table>
                  {masterData.salaryRecords.length === 0 && (
                    <div className="p-8 text-center text-slate-400">No salary records found</div>
                  )}
                </div>
              </TabsContent>
              ...
              <TabsContent value="notifications">
                <div className="rounded-md border border-slate-700">
                    ...
                    <TableBody>
                      {masterData.pendingITNotifications.map((notification) => (
                        <TableRow key={notification.id}>
                          ...
                          <TableCell>
                            <Badge variant={notification.processed ? "default" : "destructive"}>
                  ...
                  {masterData.pendingITNotifications.length === 0 && (
                    <div className="p-8 text-center text-slate-400">No pending IT notifications found</div>
                  )}
                </div>
              </TabsContent>
            </Tabs>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
              <div>
                ...
                <div className="font-medium">
                  <Badge variant={masterData.authInfo.isAuthenticated ? "default" : "destructive"}>
                    {masterData.authInfo.isAuthenticated ? "Authenticated" : "Not Authenticated"}
                  </Badge>
                </div>
              </div>
              <div>
                <span className="text-slate-400">User Role:</span>
                <div className="font-medium text-white">{masterData.authInfo.userRole || "None"}</div>
              </div>
              <div>
                <span className="text-slate-400">Current User:</span>
                <div className="font-medium text-white">{masterData.authInfo.currentUser || "None"}</div>
              </div>
            </div>
          </CardContent>
        </Card>
